...
//  const navigate = useNavigate();
  ...
  const rpcUrl = "https://goerli.optimism.io";
  // const rpcUrl = "https://endpoints.omniatech.io/v1/op/goerli/public";
  ...
  async function loadWaste() {
    ...
    console.log("Project data fetched from contract", data);
    /*
  ...
  return (
    <div className="flex justify-center bg-green-700 mb-12">
            
      <div className="px-4" style={{ maxWidth: "1600px" }}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4">
          {nfts.map((nft, i) => (

            <div key={i} className="shadow rounded-xl overflow-hidden border-2 border-white-500 mb-3">
              <img
                title="Waste"
                frameBorder="0"
                scrolling="no"
              ...
              <div className="p-1">
                ...
                <div style={{ height: "40px", overflow: "hidden" }}>
                  <p className="text-yellow-400">Location: {nft.collectionPoint}</p>
                </div>
                <p className="text-xl font-bold text-white"> Impact: {nft.weight}</p>
                <p className="text-xl font-bold text-black">Project Cost : {nft.price} USD</p>
              </div>
{/**
              <div className="p-2 bg-black">
                <button type="button" className="mt-4 w-full bg-green-500 text-white font-bold py-2 px-12 rounded" 
                        onClick={() => {
                          sendHbar(client, myAccountId, AccountId.fromEvmAddress(0, 0, metamaskAccountAddress), 2, myPrivateKey)
                        }}>
                  Contribute
                </button>
              </div>
